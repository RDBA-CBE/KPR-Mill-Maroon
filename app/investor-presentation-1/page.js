   ...
   const [state, setState] = useSetState({
    ...
    const getData = async () => {
      try {
        ...
        const res = await Models.auth.documentList(13);
                  ...
                  {state.loading ? ( 
                    ...
                    <div className=" table-responsive-sm">
                      {[...state.data?.entries()]?.map(([year, reports]) => ( 
                        <table class="table  big-table table-striped"  key={year}>
                        <thead>
                          <tr>
                            <th colspan="2" style={{ backgroundColor: "#daad19", color: "white", fontSize: "18px" }}>Investor Presentation {year}</th>
                          </tr>
                        </thead>
                        <tbody>
                                {reports?.map((report) => (
